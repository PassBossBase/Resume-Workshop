...
`buildResumePages()` 使用纯文本行数估算条目高度，并为第一页和后续页使用不同预算。它是启发式分页，不是浏览器排版测量。修改模板字号、间距或内容结构时必须同步检查分页预算和浏览器预览效果。

A4 模板基准尺寸是 `794 × 1123` CSS 像素；PDF 输出尺寸是 `210 × 297 mm`。导出时将预览节点以 `pixelRatio: 2` 转为 PNG，再按 A4 高度写入 PDF。

最近约定：编辑器右侧简历预览必须保持连续流式长页，并用红色虚线标识 A4 分页位置；红色分页标识只服务预览，不属于简历内容。PDF 导出必须使用右侧预览中的同一个连续 `.resume-page` 节点，保证导出内容与预览版式一致，同时确保红色分页标识位于导出节点之外或带有 `data-pdf-exclude="true"`，不要出现在 PDF 中。连续预览转多页 PDF 时，切片边界必须优先避开文字行、标题和图片，避免 WPS 或其他 PDF 预览工具的页间距把文字行撕开；但不要把整段、整篇 `article` 或整个信息模块提前搬到下一页，两个 PDF 页面连接处不允许出现大面积人为留白。除非需求明确改变，不要为了导出重新启用隐藏的分页模板 DOM 或让导出结果与右侧预览产生版式差异。
...
- 模型、Store、持久化、富文本或分页：浏览器中手动验证相关功能。
